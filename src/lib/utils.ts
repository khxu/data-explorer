...
export function hasDuckDbTimestampValues(result: {
  ...
  return (
    result.column_types.some(isTimestampColumnType) ||
    result.rows.some((row) =>
      row.some(
        (value) =>
          typeof value === "string" && parseDuckDbTimestampDebugString(value),
      ),
    )
  );
}

const DUCKDB_TIMESTAMP_PATTERN =
  /^Timestamp\((Second|Millisecond|Microsecond|Nanosecond), (-?\d+)\)$/;
type DuckDbTimestampUnit =
  | "Second"
  | "Millisecond"
  | "Microsecond"
  | "Nanosecond";

function formatDuckDbTimestampAsIso(
  value: unknown,
  columnType: string | undefined,
) {
  if (typeof value !== "string") return null;
  if (
    !isTimestampColumnType(columnType) &&
    !parseDuckDbTimestampDebugString(value)
  )
    return null;
  ...
  const fractional =
    nanos === 0n
      ? ""
      : `.${nanos.toString().padStart(9, "0").replace(/0+$/, "")}`;
